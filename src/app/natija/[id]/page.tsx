...
export default async function ResultPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="px-5 pb-6">
      ...
      <section
        className="rounded-[28px] p-5 text-white shadow-[0_20px_40px_-24px_rgba(2,142,17,.5)]"
      ...
      >
        ...
        <h2 className="mt-1 text-[26px] font-black leading-tight">{d.diseaseName}</h2>
      ...
      <section className="ios-card p-5">
        <p className="whitespace-pre-line text-[16px] leading-[1.6] text-[var(--brand-ink)]">
          {d.solution}
        </p>
      </section>
      ...
      <section className="ios-card">
        {meds.length === 0 && <p className="p-4 text-[var(--brand-muted)]">Dori tavsiya etilmadi.</p>}
        <ul>
          {meds.map((m) => (
            <li
              key={m}
            ...
            >
              ...
              <span className="flex-1 text-[15px] font-semibold text-[var(--brand-ink)]">{m}</span>
            </li>
          ))}
        </ul>
      </section>
      ...
      <Link href={mapHref} className="mt-6 block">
